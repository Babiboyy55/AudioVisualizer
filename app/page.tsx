...
export default function Home() {
  ...
  const currentThemeColorRef = useRef('#39FF14'); // Neon Green default
  ...
  useEffect(() => {
    ...
    const colorThemes = {
      'neon-green': { primary: '#39FF14', secondary: '#00FF00', tertiary: '#90EE90' },
      'neon-blue': { primary: '#00D9FF', secondary: '#0080FF', tertiary: '#87CEEB' },
      'neon-pink': { primary: '#FF10F0', secondary: '#FF69B4', tertiary: '#FFB6C1' },
      'neon-purple': { primary: '#BF00FF', secondary: '#9D00FF', tertiary: '#DA70D6' },
      'neon-orange': { primary: '#FF6600', secondary: '#FF8C00', tertiary: '#FFA500' },
      'neon-red': { primary: '#FF073A', secondary: '#FF0000', tertiary: '#FF6B6B' },
      'neon-yellow': { primary: '#FFFF00', secondary: '#FFD700', tertiary: '#FFEB3B' },
      'cyber-cyan': { primary: '#00FFFF', secondary: '#00CED1', tertiary: '#AFEEEE' },
      'matrix-green': { primary: '#00FF41', secondary: '#39FF14', tertiary: '#90EE90' },
      'custom': { primary: currentThemeColorRef.current, secondary: currentThemeColorRef.current, tertiary: currentThemeColorRef.current }
    };

    // Helper function to convert hex to rgba
    const hexToRgba = (hex: string, alpha: number = 1) => {
      const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);
      if (!result) return `rgba(57, 255, 20, ${alpha})`;
    ...
    const applyThemeColor = (themeName: keyof typeof colorThemes) => {
      let theme;
      
      // Handle custom color separately to get fresh value
      if (themeName === 'custom') {
        const customColor = currentThemeColorRef.current;
        theme = { 
          primary: customColor, 
      ...
      // Update current color reference
      currentThemeColorRef.current = theme.primary;
      ...
      // Update derived CSS variables for UI components
      // Convert hex to rgba for derived colors
      const hexToRgbaHelper = (hex: string, alpha: number) => {
        const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);
        if (!result) return `rgba(57, 255, 20, ${alpha})`;
        return `rgba(${parseInt(result[1], 16)}, ${parseInt(result[2], 16)}, ${parseInt(result[3], 16)}, ${alpha})`;
      };
      
      document.documentElement.style.setProperty('--grid-color', hexToRgbaHelper(theme.primary, 0.05));
      document.documentElement.style.setProperty('--panel-border', hexToRgbaHelper(theme.primary, 0.3));
      document.documentElement.style.setProperty('--panel-highlight', hexToRgbaHelper(theme.primary, 0.1));
      document.documentElement.style.setProperty('--scanner-line', hexToRgbaHelper(theme.primary, 0.7));
      
      // Update Three.js anomaly color
      if (anomalyObjectRef.current) {
        anomalyObjectRef.current.children.forEach((child: any) => {
          if (child.material && child.material.uniforms && child.material.uniforms.color) {
            child.material.uniforms.color.value = new THREE.Color(theme.primary);
          }
        });
      } else {
        // If anomaly object doesn't exist yet, create it with the new color
        updateGlowRef.current = createAnomalyObject();
      }
      
      // Update point lights colors
      if (pointLight1Ref.current) {
        pointLight1Ref.current.color = new THREE.Color(theme.primary);
      }
      if (pointLight2Ref.current) {
        pointLight2Ref.current.color = new THREE.Color(theme.primary);
      }
      
      // Recreate background particles with new colors
      updateParticlesRef.current = createBackgroundParticles();
      
      // Recreate floating particles with new colors
      if (floatingParticlesRef.current) {
        initFloatingParticles();
      }
      
      showNotification(`Theme Changed: ${themeName}`);
    };
    ...
    function setupExpandingCirclesPreloader() {
      ...
      function animate(timestamp: number) {
        ...
        for (let c = 0; c < circleCount; c++) {
          ...
          ctx!.strokeStyle = hexToRgba(themeColor, opacity * 0.2);
          ctx!.lineWidth = 1;
          ctx!.stroke();
          for (let i = 0; i < dotCount; i++) {
            ...
            ctx!.lineTo(x, y);
            ctx!.strokeStyle = hexToRgba(themeColor, opacity * 0.1);
            ctx!.lineWidth = 1;
            ctx!.stroke();
            ctx!.beginPath();
            ctx!.arc(x, y, size, 0, Math.PI * 2);
            ctx!.fillStyle = hexToRgba(themeColor, opacity * 0.9);
            ctx!.fill();
          }
        }
    ...
    function initFloatingParticles() {
      ...
      for (let i = 0; i < numParticles; i++) {
        ...
        // Use theme color with slight variations
        ...
        particle.style.backgroundColor = `rgba(${Math.max(0, Math.min(255, rgb.r + rVariation))
          }, ${Math.max(0, Math.min(255, rgb.g + gVariation))
          }, ${Math.max(0, Math.min(255, rgb.b + bVariation))
          }, ${Math.random() * 0.5 + 0.2})`;
      ...
      try {
        ...
        audioPlayer.onended = function () {
          ...
          if (playPauseBtnRef.current) {
            playPauseBtnRef.current.textContent = "▶ Play";
          }
        };
    ...
    function drawCircularVisualizer() {
      ...
      circularCtx.fillStyle = hexToRgba(themeColor, 0.05);
      circularCtx.fill();
      const numRings = 3;
      for (let ring = 0; ring < numRings; ring++) {
        ...
        const darkerRgb = { r: Math.floor(rgb.r * 0.7), g: Math.floor(rgb.g * 0.7), b: Math.floor(rgb.b * 0.7) };
        const lighterRgb = { r: Math.floor(rgb.r * 1.2), g: Math.floor(rgb.g * 1.2), b: Math.floor(rgb.b * 1.2) };
        ...
        if (ring === 0) {
          ...
          gradient.addColorStop(1, `rgba(${darkerRgb.r}, ${darkerRgb.g}, ${darkerRgb.b}, ${opacity * 0.7})`);
        } else if (ring === 1) {
          gradient = circularCtx.createRadialGradient(
            centerX,
            centerY,
          ...
          gradient.addColorStop(0, `rgba(${darkerRgb.r}, ${darkerRgb.g}, ${darkerRgb.b}, ${opacity})`);
          gradient.addColorStop(1, `rgba(${lighterRgb.r}, ${lighterRgb.g}, ${lighterRgb.b}, ${opacity * 0.7})`);
        } else {
          gradient = circularCtx.createRadialGradient(
            centerX,
            centerY,
          ...
          gradient.addColorStop(0, `rgba(${lighterRgb.r}, ${lighterRgb.g}, ${lighterRgb.b}, ${opacity})`);
          gradient.addColorStop(1, `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${opacity * 0.7})`);
        }
        circularCtx.strokeStyle = gradient;
        circularCtx.lineWidth = 2 + (numRings - ring);
        circularCtx.stroke();
        circularCtx.shadowBlur = 15;
        circularCtx.shadowColor = hexToRgba(themeColor, 0.7);
      }
    ...
    function drawSpectrumAnalyzer() {
      ...
      spectrumCtx.strokeStyle = hexToRgba(themeColor, 0.2);
      ...
      spectrumCtx.fillStyle = hexToRgba(themeColor, 0.7);
    ...
    function updateAudioWave() {
      ...
      const themeColor = getThemeColor();
      wave.style.borderColor = hexToRgba(themeColor, 0.1 + normalizedAverage * 0.3);
    }
    ...
    function drawWaveform() {
      ...
      if (audioAnalyserRef.current && audioDataRef.current) {
        ...
        waveformCtx.strokeStyle = hexToRgba(themeColor, 0.8);
    ...
    function initThreeJS() {
      ...
      // Get current theme color for lights
      const currentColor = getThemeColor();
      
      const pointLight1 = new THREE.PointLight(new THREE.Color(currentColor), 1, 10);
      pointLight1.position.set(2, 2, 2);
      sceneRef.current.add(pointLight1);
      const pointLight2 = new THREE.PointLight(new THREE.Color(currentColor), 1, 10);
      pointLight2.position.set(-2, -2, -2);
      sceneRef.current.add(pointLight2);
      
      // Store lights for color updates
    ...
    function createBackgroundParticles() {
      ...
      if (backgroundParticlesRef.current && sceneRef.current) {
      ...
      const secondaryColor = getComputedStyle(document.documentElement)
        .getPropertyValue('--accent-secondary').trim() || '#00FF00';
      const tertiaryColor = getComputedStyle(document.documentElement)
        .getPropertyValue('--accent-tertiary').trim() || '#90EE90';
      ...
      backgroundParticlesRef.current = particles; // Save reference
    ...
    function createAnomalyObject() {
      ...
      const outerMaterial = new THREE.ShaderMaterial({
        uniforms: {
          ...
          color: {
            value: new THREE.Color(currentColor)
          },
          audioLevel: {
            value: 0
          },
    ...
    const handleFileChange = (e: Event) => {
      ...
      if (files && files.length > 0) {
        ...
        initAudioFile(file);
        if (playPauseBtnRef.current) {
          playPauseBtnRef.current.textContent = "⏸ Pause";
        }
      }
    };
    ...
    const handlePlayPause = () => {
      const audioElement = currentAudioElementRef.current || audioPlayerRef.current;
      if (!audioElement || !audioElement.src) {
        showNotification("No Audio File Loaded");
        return;
      }
      ...
      if (audioElement.paused) {
        ...
        audioElement.play().then(() => {
          ...
          showNotification("Audio Play Error");
        });
      } else {
        audioElement.pause();
        isAudioPlayingRef.current = false;
    ...
    playPauseBtnRef.current?.addEventListener("click", handlePlayPause);
    ...
    stopBtnRef.current?.addEventListener("click", handleStop);

    const handleVolumeChange = (e: Event) => {
      const volume = parseFloat((e.target as HTMLInputElement).value);
      if (volumeValueRef.current) {
        volumeValueRef.current.textContent = Math.round(volume * 100).toString();
      }
      const audioElement = currentAudioElementRef.current || audioPlayerRef.current;
      if (audioElement) {
    ...
    volumeSliderRef.current?.addEventListener("input", handleVolumeChange);

    const updateProgress = () => {
      const audioElement = currentAudioElementRef.current || audioPlayerRef.current;
      if (!audioElement || !progressFillRef.current || !timeDisplayRef.current) {
        return;
      }
    ...
    const handleProgressClick = (e: MouseEvent) => {
      ...
      if (!audioElement || !progressBar || !audioElement.duration) {
        return;
      }
    ...
    progressBarRef.current?.addEventListener("click", handleProgressClick as any);

    const handleThemeChange = (e: Event) => {
      const themeName = (e.target as HTMLSelectElement).value as keyof typeof colorThemes;
      
      // Show/hide custom color picker
      if (customColorPickerRef.current) {
        customColorPickerRef.current.style.display = themeName === 'custom' ? 'block' : 'none';
        
        // Sync color picker value with current theme color when switching to custom
        if (themeName === 'custom') {
          customColorPickerRef.current.value = currentThemeColorRef.current;
        }
      }
    ...
    const handleCustomColorChange = (e: Event) => {
      ...
      // Update the current color reference FIRST
      currentThemeColorRef.current = color;
      // Then apply the custom theme which will use the new color
      applyThemeColor('custom');
    };

    themeSelectRef.current?.addEventListener("change", handleThemeChange);
    customColorPickerRef.current?.addEventListener("input", handleCustomColorChange);

    const audioElement = currentAudioElementRef.current || audioPlayerRef.current;
    if (audioElement) {
      audioElement.addEventListener("play", startProgressTracking);
      audioElement.addEventListener("pause", stopProgressTracking);
      audioElement.addEventListener("ended", stopProgressTracking);
    }
    ...
    return () => {
      ...
      playPauseBtnRef.current?.removeEventListener("click", handlePlayPause);
      stopBtnRef.current?.removeEventListener("click", handleStop);
      volumeSliderRef.current?.removeEventListener("input", handleVolumeChange);
      progressBarRef.current?.removeEventListener("click", handleProgressClick as any);
      
      const audioElement = currentAudioElementRef.current || audioPlayerRef.current;
      if (audioElement) {
        audioElement.removeEventListener("play", startProgressTracking);
        audioElement.removeEventListener("pause", stopProgressTracking);
        audioElement.removeEventListener("ended", stopProgressTracking);
      }
      stopProgressTracking();
      
      themeSelectRef.current?.removeEventListener("change", handleThemeChange);
      customColorPickerRef.current?.removeEventListener("input", handleCustomColorChange);
      ...
      <style jsx global>{`
        ...
        * {
        ...
        .audioWave {
          ...
          border: 0.0625rem solid var(--panel-highlight);
          pointer-events: none;
          z-index: 3;
        }
        .audioWave::before {
      ...
      <div className="controlPanel" ref={controlPanelRef}>
        ...
        <div className="controlGroup">
          <div className="controlRow">
            <span className="controlLabel">Color Theme</span>
          </div>
          <select className="themeSelect" id="themeSelect" ref={themeSelectRef} defaultValue="neon-green">
      ...
      <div className="spectrumAnalyzer" ref={spectrumAnalyzerRef}>
        ...
        <div className="audioControls" ref={audioControlsRef}>
          ...
          <div className="playbackControls">
            <button className="playPauseBtn" id="playPauseBtn" ref={playPauseBtnRef}>▶ Play</button>
            <button className="stopBtn" id="stopBtn" ref={stopBtnRef}>⏹ Stop</button>
          </div>
          
          <div className="progressBarContainer" ref={progressBarRef}>
            <div className="progressBarFill" ref={progressFillRef}></div>
          </div>
          <div className="timeDisplay" ref={timeDisplayRef}>0:00 / 0:00</div>
          ...
          <div className="volumeControl">
            <div className="controlRow">
              ...
              <span className="controlValue" id="volumeValue" ref={volumeValueRef}>100</span>
            </div>
            <input type="range" min="0" max="1" defaultValue="1" step="0.01" className="slider" id="volumeSlider" ref={volumeSliderRef} />
          </div>
        </div>
      </div>
    </>
